...
import { applyEvent, fresh, give, faction, turkey, PACT, fireAt, group, setVariable, variable } from './helpers'
...
describe('the population exchange', () => {
  test('disrupts regional transport without creating or removing units', () => {
    const g = fresh()
    give(g, 'izmir', turkey(g))
    give(g, 'aydin', turkey(g))
    g.turn.configure({ round: 16 })
    const before = g.territories.reduce((sum, territory) => sum + territory.troops, 0)
    applyEvent(g, 'event.mubadele')
    assert.equal(variable(g, 'mubadele.disruptionRound'), 16)
    assert.equal(
      g.territories.reduce((sum, territory) => sum + territory.troops, 0),
      before,
    )
  })

  test('respects Western Thrace’s treaty exemption', () => {
    assert.ok(!group('mubadeleAffected').includes('western-thrace'))
  })
})

describe('political successions', () => {
  test('replaces leaders in the HUD after their fall', () => {
    const g = fresh()
    const greekPlayer = g.players.find((player) => player.faction.name === 'Greece')!
    const britishPlayer = g.players.find((player) => player.faction.name === 'Britain')!
    assert.equal(g.leaderName(greekPlayer), 'Eleftherios Venizelos')
    assert.equal(g.leaderName(britishPlayer), 'David Lloyd George')

    applyEvent(g, 'event.venizelos')
    applyEvent(g, 'event.lloydGeorge')
    assert.equal(g.leaderName(greekPlayer), 'Dimitrios Rallis')
    assert.equal(g.leaderName(britishPlayer), 'Andrew Bonar Law')
  })
})
