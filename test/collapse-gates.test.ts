...
describe('the population exchange needs both countries', () => {
  test('does not fire after Greece has left the map', () => {
    const g = fresh()
    fireAt(g, 5)
    const greece = faction(g, 'Greece')
    for (const territory of [...greece.territories]) give(g, territory.slug, turkey(g))
    assert.equal(greece.eliminated, true)
    assert.ok(!fireAt(g, 16).includes('event.mubadele'))
  })
})
