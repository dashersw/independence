...
import Game, { HISTORICAL_EVENTS } from '../src/game/game'
import factionData from '../src/game/factions.json'
import { fresh, give, faction, turkey, fireAt, fireThrough, roundOfEvent, PACT } from './helpers'
...
describe('an occupier that is winning does not collapse', () => {
  ...
  test('Britain at double its start does not stand down', () => {
    ...
    assert.equal(g.britainStoodDown, false)
...
describe('the Allies reassert their hold on the City', () => {
  ...
  test('the Assembly is untouched — it never sat in İstanbul', () => {
    ...
    g.occupyIstanbul()
    g.assemblyUpkeep()
    assert.equal(g.assemblyOpened, true, 'Ankara is the seat, and Ankara is still Turkish')
  })
  ...
  test('with Britain gone the City stays where it is', () => {
    ...
    britain.eliminated = true
    g.occupyIstanbul()
    assert.equal(g.bySlug['istanbul'].faction, turkey(g), 'nobody left to send an occupation force')
    assert.equal(g.bySlug['istanbul'].troops, 9)
  })

  test('the card keeps trying through 1920, then retires', () => {
    const g = fresh()
    const event = HISTORICAL_EVENTS.find(e => e.textKey === 'event.istanbulOccupied')!
    assert.equal(event.attempts, 3, 'the occupation was continuous, not a single quarter')
  ...
  test('and it cannot lie in wait for a City taken years later', () => {
    ...
    g.round = 16
    give(g, 'istanbul', turkey(g))
    assert.equal(event.gate!(g), false, 'by 1923 the occupation is a spent force')
  })
})
