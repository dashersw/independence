...
import { attackMoves, chooseMove, fortifyMoves, reinforceMoves, playTurn } from '../src/ai/policy'
...
describe('what the network sees', () => {
  ...
  test('the odds feature tracks the odds', () => {
    ...
    for (const [a, d] of [[0, 0], [1, 0], [99, 0], [3, 99]]) {
  ...
  test('the delta block is zero for a move that changes nothing', () => {
    ...
    assert.ok(delta.every(v => v === 0), 'end moves nothing, so it changes nothing')
  })
...
describe('what each faction is paid for', () => {
  ...
  test('a faction scripted out of the war is not scored as a rout', () => {
    ...
    assert.equal(g.atPeace(italy), true, 'it left rather than lost')
...
describe('playing a turn through a model', () => {
  ...
  test('it only ever offers moves the engine would accept', () => {
    ...
    for (const move of attackMoves(g, turkey(g))) {
      ...
      assert.ok(g.attackTargets(move.from!.slug).includes(move.to!.slug))
    }
  })
  ...
  test('stopping is always on the table', () => {
    const g = fresh()
    g.phase = 'attack'
    assert.ok(attackMoves(g, turkey(g)).some(m => m.kind === 'end'), 'it may decline to attack')
    assert.ok(fortifyMoves(g, turkey(g)).some(m => m.kind === 'end'), 'and decline to move')
  })

  test('a faction with nothing to move offers only the option to stand still', () => {
    const g = fresh()
    for (const t of turkey(g).territories) t.troops = 1
    assert.deepEqual(fortifyMoves(g, turkey(g)).map(m => m.kind), ['end'])
  })
  ...
  test('a whole turn leaves the game in a legal state', () => {
    ...
    playTurn(g, model, { maxAttacks: 4 })
    assert.notEqual(g.currentPlayer.faction, before, 'the turn passed on')
    assert.equal(g.pendingAdvance, null, 'nothing is left hanging')
  ...
  test('it never leaves reinforcements undeployed', () => {
    ...
    const owed = g.reinforcementsLeft
    const before = faction.troopTotal
    assert.ok(owed > 0, 'it has men to place')
    playTurn(g, model, { maxAttacks: 0 })
    ...
    assert.equal(faction.troopTotal + g.troopsAtSea(faction), before + owed, 'every one of them went somewhere')
  })
})
...
describe('the stretch goals', () => {
  ...
  test('Turkey is graded on the ending it actually reached', () => {
    ...
    assert.equal(terminal(poor, turkey(poor)), TURKEY_ENDINGS['overlay.lausanne.poor.title'])
  })
...
describe('a model-driven faction is not gagged by the written AI\u2019s personality', () => {
  const model = new Net([INPUT_SIZE, 8, 1], 3).toJSON()

  test('the hand-written attack caps do not apply once a model is driving', () => {
    // Britain is written as two strikes a turn and Italy as one, which is how a
    // written AI is told that London will not campaign in Anatolia. A trained
    // one is told by its rewards instead, and needs the room to act on them.
    for (const name of ['Britain', 'Italy', 'Armenia']) {
      const g = new Game()
      const seat = g.players.findIndex(p => p.faction.name === name)
      g.currentPlayerIndex = seat
      g.startTurn()
      g.aiBeginTurn()
      const written = g.aiAttacksLeft

      const m = new Game()
      m.aiScorer = () => 0
      m.currentPlayerIndex = seat
      m.startTurn()
      m.aiBeginTurn()
      assert.ok(m.aiAttacksLeft > written, `${name}: ${m.aiAttacksLeft} under a model vs ${written} written`)
    }
  })
  ...
  test('but the treaties that end their wars still do', () => {
    ...
    g.round = 20 // long past the evacuation
    g.currentPlayerIndex = seat
    g.startTurn()
    g.aiBeginTurn()
    assert.equal(g.isPassive(g.currentPlayer.faction), true)
    assert.equal(g.aiAttacksLeft, 0, 'a power that has left the war does not attack')
  })
