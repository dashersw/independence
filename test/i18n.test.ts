...
/** Keys declared in each language block, read from the source. */
const keysByLang = (() => {
  const en = source.indexOf('  en: {')
  const tr = source.indexOf('  tr: {')
  assert.ok(en > -1 && tr > en, 'could not find the language blocks')
  const grab = (chunk: string) => new Set([...chunk.matchAll(/^\s{4}'([^']+)':/gm)].map(m => m[1]))
  return { en: grab(source.slice(en, tr)), tr: grab(source.slice(tr)) }
})()
...
describe('dictionary parity', () => {
  ...
  test('every event has copy in both languages', () => {
    for (const e of HISTORICAL_EVENTS) {
      assert.ok(keysByLang.en.has(e.textKey), `${e.textKey} has no English copy`)
      assert.ok(keysByLang.tr.has(e.textKey), `${e.textKey} has no Turkish copy`)
    }
  })
  ...
  test('the decision has copy for every branch', () => {
    ...
    for (const choice of decision?.choices ?? []) {
      ...
      assert.ok(keysByLang.en.has(`${decision?.textKey}.${choice.key}.log`), `${choice.key} log line`)
