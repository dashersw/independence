...
import { fresh, give, faction, turkey, drive, entrenchTurkey, Fired, variable } from './helpers'
...
const historical = () => {
  ...
  for (const slug of ['kars', 'igdir']) give(g, slug, turkey(g)).troops = 400
  return g
}
...
describe('the historical line, played end to end', () => {
  test('every event arrives on its own turn, in order, once', () => {
    ...
    for (const [key, round] of expected)
      assert.equal(roundOfFirst(fired, key), round, `${key} fired late or not at all`)
  ...
  test('the campaign leaves the expected cumulative state', () => {
    const g = historical()
    drive(g, 17)
    assert.equal(variable(g, 'assembly.active'), true, 'the Assembly sat and stayed')
    assert.equal(variable(g, 'treaties.kars.signed'), true)
    assert.ok(Number(variable(g, 'sakarya.round')) > 0, 'Sakarya was fought')
    assert.equal(variable(g, 'britain.stoodDown'), true)
    assert.equal(variable(g, 'greece.collapsed'), true)
    assert.equal(variable(g, 'government.fortifyBonus'), 1, 'the Sultanate was abolished')
...
describe('scenario: the government is driven out mid-war', () => {
  test('the economy collapses, the offensive waits, and both return together', () => {
    ...
    const fired = drive(g, 20, {
      onRound: (game) => {
        if (game.turn.round === 9) levyWithGovernment = game.campaign.reinforcementsFor(turkey(game))
        if (game.turn.round === 10) {
          // the seats fall
        ...
        if (game.turn.round === 12) levySuspended = game.campaign.reinforcementsFor(turkey(game))
        if (game.turn.round === 15) {
    ...
    assert.equal(variable(g, 'assembly.active'), true, 'and the Assembly is back by the end')
  })
  ...
  test('a treaty already signed survives the collapse', () => {
    ...
    drive(g, 12, {
      onRound: (game) => {
        if (game.turn.round === 12) {
          give(game, 'ankara', greece, 12)
          give(game, 'sivas', greece, 12)
        }
      },
    })
    assert.equal(variable(g, 'treaties.kars.signed'), true, 'Kars was signed at 11 and stays signed')
  })
})
...
describe('scenario: the requisition window', () => {
  test('opens, boosts, costs, and closes on schedule', () => {
    ...
    drive(g, 15, {
      orders: true,
      onRound: (game) => {
        levies[game.turn.round] = game.campaign.reinforcementsFor(turkey(game))
      },
    })
    assert.equal(variable(g, 'tekalif.until'), 12, 'proclaimed at 10, runs through 12')
  ...
  test('declining leaves the levy alone', () => {
    ...
    drive(a, 12, { orders: true, onRound: (g) => (levyA[g.turn.round] = g.campaign.reinforcementsFor(turkey(g))) })
    drive(b, 12, { orders: false, onRound: (g) => (levyB[g.turn.round] = g.campaign.reinforcementsFor(turkey(g))) })
    assert.ok(levyA[11] < levyB[11], 'the orders cost reinforcements the decline does not')
    assert.equal(variable(b, 'tekalif.until'), 0)
  })
})
...
describe('scenario: İstanbul changes hands twice', () => {
  ...
  test('retaking İstanbul yourself blocks Mudanya entirely', () => {
    ...
    const fired = drive(g, 16, {
      onRound: (game) => {
        if (game.turn.round === 8 && game.bySlug['istanbul'].faction.name !== 'Turkey') {
...
describe('scenario: the Mosul question decides the ending', () => {
  test('signing at the conference short of Musul lands the near-miss card', () => {
    ...
    assert.equal(gameOutcome(g)?.vars.held, 29)
  })
  ...
  test('refusing keeps the war alive long enough for the League to award Musul', () => {
    ...
    assert.ok(Number(variable(g, 'conference.rejectedAt')) > 0, 'terms were refused')
...
describe('scenario: a war interrupted by a save', () => {
  test('resuming mid-campaign produces the same remaining events', () => {
    ...
    assert.ok(restored.turn.round > a.turn.round, 'the resumed game advanced')
...
describe('scenario: breaking a peace you were offered', () => {
  test('attacking Italy early does NOT stop the evacuation — there is no peace to break yet', () => {
    ...
    const fired = drive(g, 12, {
      onRound: (game) => {
        if (game.turn.round !== 9) return
        assert.equal(game.campaign.atPeace(italy), false, 'Italy only settles at round 10')
        ...
        game.turn.configure({ playerIndex: game.players.findIndex((p) => p.isHuman) })
        target.troops = 1
        game.combat.begin(own.slug, target.slug)
        game.combat.blitz(own.slug, target.slug)
        game.turn.configure({ phase: 'fortify' })
      },
    })
  ...
  test('the concession is applied before any seat can act on the round it lands', () => {
    ...
    drive(g, 10, {
      onRound: (game) => {
        if (game.turn.round !== 10) return
        assert.equal(game.campaign.atPeace(italy), true, 'now at peace')
  ...
  test('a provoked Armenia mobilises again but still cannot cross the Kars line', () => {
    ...
    drive(g, 12)
    assert.equal(variable(g, 'treaties.kars.signed'), true)
    assert.equal(g.campaign.reinforcementsFor(armenia), 0, 'Gümrü demobilised them')
    armenia.peaceBroken = true
    assert.ok(g.campaign.reinforcementsFor(armenia) > 0, 'breaking the peace puts them back in the field')
    give(g, 'gyumri', turkey(g), 12)
    assert.equal(
      g.campaign.frontClosed(armenia, g.bySlug['gyumri']),
      false,
      'a lost homeland province can be recovered',
    )
    for (const slug of ['kars', 'igdir', 'erzurum', 'van', 'trabzon', 'sivas'])
      assert.equal(g.campaign.frontClosed(armenia, g.bySlug[slug]), true, `${slug} stays beyond the treaty line`)
  })
})
...
describe('scenario: Sakarya and what follows', () => {
  test('freeze, permanent dice cap, then the collapse', () => {
    ...
    const fired = drive(g, 18, {
      onRound: (game) => {
        if (game.campaign.frozen(greece)) frozenOn.push(game.turn.round)
      },
    })
    ...
    if (greek && target) {
      g.turn.configure({ round: 17 })
      assert.equal(g.combat.diceCaps(greek, target).attacker, 2, 'still capped years later')
    }
...
describe('scenario: the requisition and the militia clock', () => {
  test('the orders freeze militia growth, and it never comes back', () => {
    ...
    drive(g, 16, {
      orders: true,
      onRound: (game) => {
        readings[game.turn.round] = sivas.entrenched
      },
    })
  ...
  test('declining keeps the third tick on schedule', () => {
    ...
    const sivas = g.bySlug['sivas']
    const readings: Record<number, number> = {}
    drive(g, 14, { orders: false, onRound: (game) => (readings[game.turn.round] = sivas.entrenched) })
...
describe('scenario: Çerkes Ethem sets the west back', () => {
  test('the revolt halves garrisons and resets their dug-in progress', () => {
    ...
    drive(g, 9, {
      onRound: (game) => {
        if (game.turn.round === 7) before = Object.fromEntries(west.map((s) => [s, game.bySlug[s].troops]))
      },
    })
...
describe('scenario: the war is lost', () => {
  test('losing both seats permanently costs the offensive and ends at a graded peace', () => {
    ...
    assert.equal(variable(g, 'assembly.active'), false, 'no government ever sat')
    assert.ok(!keys(fired).includes('event.greatOffensive'), 'and so no offensive')
    assert.equal(g.turn.phase, 'gameover')
    const outcome = gameOutcome(g)
    assert.ok(outcome)
    assert.ok(outcome.titleKey.startsWith('overlay.lausanne'), outcome.titleKey)
    assert.notEqual(gameOutcome(g)?.titleKey, 'overlay.victory.title')
  })
...
describe('scenario: the whole map', () => {
  test('taking everything ends the war on the absolute-victory card', () => {
    const g = entrenchTurkey(fresh())
    drive(g, 6)
    for (const t of g.territories) if (t.faction.name !== 'Turkey') give(g, t.slug, turkey(g), g.turn.round)
    g.checkGameEnd()
