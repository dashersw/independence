...
describe('staging a battle', () => {
  test('beginAttack rolls no dice', () => {
    ...
    assert.equal(g.turn.attacksUsed, 0, 'selecting a target does not spend an attack')
  })

  test('cancelling a staged battle has no attack or diplomatic consequences', () => {
    const g = fresh()
    g.turn.configure({ round: 12 })
    const italy = faction(g, 'Italy')
    setVariable(g, 'withdrawals.Italy', true)
    const own = turkey(g).territories.find((territory) => territory.adjacent.some((next) => next.faction === italy))
    const target = own?.adjacent.find((territory) => territory.faction === italy)
    if (!own || !target) return
    stageAttack(g, own.slug, target.slug, 20, 3)

    assert.ok(g.combat.begin(own.slug, target.slug))
    g.combat.pullBack()

    assert.equal(g.turn.attacksUsed, 0)
    assert.equal(italy.peaceBroken, false)
    assert.equal(italy.grudges.has('Turkey'), false)
    assert.equal(
      g.log.some((entry) => entry.key === 'log.repelled'),
      false,
    )
  })
  ...
  test('pressing an attack against a faction at peace breaks the peace', () => {
    const g = fresh()
    g.turn.configure({ round: 12 })
    const italy = faction(g, 'Italy')
    setVariable(g, 'withdrawals.Italy', true)
    ...
    assert.equal(italy.peaceBroken, false, 'target selection alone does not break the peace')
    g.combat.step(own.slug, target.slug)
...
describe('operational limits', () => {
  test('a turn permits three new battles, and dice exchanges do not consume the budget', () => {
    ...
    assert.equal(g.turn.attacksUsed, 0, 'staging does not consume the operational budget')
  ...
  test('a spent attack phase waits for the final battle to finish, then advances', () => {
    const g = fresh()
    const { from, to } = findBorder(g, (slug) => PACT.includes(slug))
    stageAttack(g, from.slug, to.slug, 40, 20)
    ...
    for (let battle = 0; battle < 3; battle++) {
      ...
      withRandom(0, () => g.combat.step(from.slug, to.slug))
      if (battle < 2) g.combat.pullBack()
    }

    assert.equal(g.turn.attacksLeft, 0)
    assert.equal(g.combat.hasActiveBattle, true)
    assert.equal(g.turn.finishAttackPhaseIfSpent(), false, 'the last battle may continue')
    assert.equal(g.turn.phase, 'attack')

    g.combat.pullBack()
    assert.equal(g.turn.finishAttackPhaseIfSpent(), true)
...
describe('resolving a battle', () => {
  ...
  test('cancelling a running battle clears it without reporting a repel', () => {
    const g = fresh()
    const { from, to } = findBorder(g, (slug) => PACT.includes(slug))
    stageAttack(g, from.slug, to.slug, 30, 20)
    g.combat.begin(from.slug, to.slug)
    g.combat.step(from.slug, to.slug)
    const attacksUsed = g.turn.attacksUsed

    g.combat.cancel()

    assert.equal(g.combat.snapshot().active, null)
    assert.equal(g.turn.attacksUsed, attacksUsed, 'cancelling does not refund an attack already made')
    assert.equal(
      g.log.some((entry) => entry.key === 'log.repelled'),
      false,
    )
