...
import { fresh, give, faction, turkey, PACT, drive, entrenchTurkey, roundOfEvent, Fired } from './helpers'
...
const advance = (g: Game, rounds: number, prep?: (g: Game) => void) => {
  for (let n = 0; n < rounds && g.phase !== 'gameover'; n++) {
    g.round++
    prep?.(g)
    g.conferenceUpkeep()
  }
...
describe('winning before the conference ever sits', () => {
  ...
  test('two turns of holding it are not enough', () => {
    const g = board(PACT.length)
    drive(g, 2)
    assert.equal(g.pactHeldTurns, 2, 'the clock is at two and still running')
  ...
  test('losing a province mid-clock resets it and costs the early win', () => {
    ...
    const fired = drive(g, 10, {
      onRound: game => {
        if (game.round === 3) give(game, 'izmir', greece, 3).troops = GARRISON
      }
    })
    assert.notEqual(g.phase, 'gameover', 'the war runs on')
  ...
  test('a Pact completed on the eve of the conference still has to be held', () => {
    ...
    const fired = drive(g, 24, {
      onRound: game => {
        if (game.round === 17) give(game, PACT[PACT.length - 1], turkey(game), 17).troops = GARRISON
      }
    })
    assert.equal(g.phase, 'gameover')
...
describe('Ankara changes hands', () => {
  test('losing both seats before the conference costs the Assembly, not the peace', () => {
    const g = board(24)
    const greece = faction(g, 'Greece')
    const fired = drive(g, 22, {
      onRound: game => {
        if (game.round !== 12) return
        give(game, 'ankara', greece, 12).troops = GARRISON
        give(game, 'sivas', greece, 12).troops = GARRISON
      }
    })
    assert.equal(g.assemblyOpened, false, 'no government by the time terms are offered')
    assert.equal(g.assemblyEverOpened, true, 'but there was one')
    assert.ok(keys(fired).includes('event.conference'), 'the conference sits regardless of who governs')
    assert.equal(g.phase, 'gameover')
    ...
    assert.equal(g.outcome?.vars.held, 21, 'Ankara, Sivas and İstanbul all off the count')
  })
  ...
  test('retaking a seat brings the Assembly back, and the conference keeps its date', () => {
    ...
    const fired = drive(g, 22, {
      onRound: game => {
        ...
        if (game.round === 13) give(game, 'ankara', turkey(game), 13).troops = GARRISON
      }
    })
    assert.equal(g.assemblyOpened, true, 'three turns holding Ankara reconvened it')
  ...
  test('a seat traded back and forth still only opens the Assembly once', () => {
    ...
    const fired = drive(g, 17, {
      onRound: game => {
        if ([7, 11, 15].includes(game.round)) {
          give(game, 'ankara', greece, game.round).troops = GARRISON
          give(game, 'sivas', greece, game.round).troops = GARRISON
        }
        if ([9, 13].includes(game.round)) give(game, 'ankara', turkey(game), game.round).troops = GARRISON
      }
    })
  ...
  test('the capital lost and retaken during the landings does not stop the win', () => {
    ...
    drive(g, FINAL, {
      terms: 'reject',
      onRound: game => {
        if (game.round === CONFERENCE + 1) give(game, 'ankara', greece, game.round).troops = GARRISON
        if (game.round !== CONFERENCE + 2) return
        for (const slug of PACT) give(game, slug, turkey(game), game.round).troops = GARRISON
      }
    })
    assert.equal(g.phase, 'gameover')
    assert.equal(g.outcome?.titleKey, 'overlay.victory.title', 'the Pact was retaken and the war won')
...
describe('accepting the terms', () => {
  ...
  test('signing one province short lands the near-miss card and names it', () => {
    ...
    assert.equal(g.outcome?.vars.missing, 1)
    assert.ok(String(g.outcome?.vars.named).length > 0)
  })
  ...
  test('nobody comes ashore in a war that was settled', () => {
    ...
    assert.deepEqual(g.landedOn, [])
    assert.equal(g.rejectedAt, 0)
...
describe('refusing the terms', () => {
  test('the war does not end, and the refusal is on the record', () => {
    ...
    assert.notEqual(g.phase, 'gameover', 'refusing buys more war, not an ending')
    assert.equal(g.pactHeldTurns, 0)
  })
  ...
  test('a defended coast throws the first wave straight back', () => {
    const g = board(20)
    drive(g, CONFERENCE, { terms: 'reject' })
    assert.equal(g.round, CONFERENCE + 1, 'the first wave has been and gone')
    assert.deepEqual(g.landedOn, [], 'thirty men against four hundred take nothing')
  })
  ...
  test('a stripped coast gives them their beachhead', () => {
    ...
    strippedCoast(g)
    advance(g, 1)
    assert.ok(g.landedOn.length > 0, 'the first wave is thirty men and it lands')
  })
  ...
  test('the landings keep coming while the refusal stands', () => {
    ...
    strippedCoast(g)
    advance(g, 1)
    const first = g.landedOn.length
    strippedCoast(g)
    advance(g, 5)
    assert.ok(g.landedOn.length > first, 'later waves take more coast')
  })
  ...
  test('a beachhead becomes a province Turkey is allowed to attack', () => {
    ...
    strippedCoast(g)
    advance(g, 3)
    assert.ok(g.landedOn.length > 0)
    for (const slug of g.landedOn)
      assert.equal(g.mayAttackOutsidePact(g.bySlug[slug]), true, `${slug} should be a legal target`)
  })
  ...
  test('a landing is a raid, so the province stays open for a few turns after', () => {
    ...
    strippedCoast(g)
    advance(g, 1)
    const slug = g.landedOn[0]
    assert.ok(slug, 'something landed')
    assert.equal(g.bySlug[slug].raidedOn, g.round)
  })
...
describe('nobody left to enforce the terms', () => {
  ...
  test('killing the powers after refusing stops the waves', () => {
    ...
    strippedCoast(g)
    advance(g, 2)
    const beachheads = g.landedOn.length
    ...
    advance(g, 4)
    assert.equal(g.landersAlive, false)
    assert.equal(g.landedOn.length, beachheads, 'nobody is left to send another fleet')
    assert.equal(g.rejectedAt, CONFERENCE, 'the refusal still stands on the record')
  })
})
...
describe('the restraint rule across a whole war', () => {
  test('Turkey may not cross the Pact line while the aim is unmet', () => {
    ...
    assert.equal(g.beginAttack(from.slug, to.slug), null, 'the order is refused')
  })
  ...
  test('being raided opens a province for a few turns, then closes it again', () => {
    ...
    assert.equal(g.mayAttackOutsidePact(to), true)
    g.round += 4
    assert.equal(g.mayAttackOutsidePact(to), false, 'the licence lapses')
  })
  ...
  test('a beachhead is a standing target, not a lapsing one', () => {
    ...
    strippedCoast(g)
    advance(g, 1)
    const slug = g.landedOn.find(s => !PACT.includes(s))
    if (!slug) return // every beach that round was inside the Pact, which is legal anyway
    g.round += 10
    assert.equal(g.mayAttackOutsidePact(g.bySlug[slug]), true)
  })
})
...
describe('the Straits decide how far the fleets get', () => {
  test('holding both sides of the Dardanelles keeps İstanbul off the list', () => {
    ...
    assert.ok(!g.landingSites().some(s => s.slug === 'istanbul'), 'the Marmara stays shut')
  })
  ...
  test('losing both opens it', () => {
    ...
    assert.ok(g.landingSites().some(s => s.slug === 'istanbul'))
  })
...
describe('the war after the conference', () => {
  ...
  test('every refused war still ends, and ends graded', () => {
    for (const held of [12, 18, 26]) {
      const g = board(held)
      drive(g, FINAL + 2, { terms: 'reject' })
      assert.equal(g.phase, 'gameover', `${held} provinces: no war may run past the final round`)
      assert.ok(g.outcome, 'and it must produce an ending')
...
describe('the Assembly seats through a long war', () => {
  test('Sivas alone keeps the Assembly sitting', () => {
    ...
    const fired = drive(g, 17, {
      onRound: game => {
        if (game.round === 9) give(game, 'ankara', greece, 9).troops = GARRISON
      }
    })
    assert.equal(g.assemblyOpened, true, 'the seat fell back to Sivas')
  ...
  test('both seats gone suspends it; Sivas alone brings it back', () => {
    ...
    drive(g, 17, {
      onRound: game => {
        seen[game.round] = game.assemblyOpened
        if (game.round === 8) {
          give(game, 'ankara', greece, 8).troops = GARRISON
          give(game, 'sivas', greece, 8).troops = GARRISON
        }
        if (game.round === 11) give(game, 'sivas', turkey(game), 11).troops = GARRISON
      }
    })
  ...
  test('an Assembly driven out mid-landing holds the Caliphate up until it comes back', () => {
    ...
    const fired = drive(g, FINAL, {
      terms: 'reject',
      onRound: game => {
        if (game.round === CONFERENCE + 1) {
          give(game, 'ankara', greece, game.round).troops = GARRISON
          give(game, 'sivas', greece, game.round).troops = GARRISON
        }
        if (game.round === CONFERENCE + 4) give(game, 'sivas', turkey(game), game.round).troops = GARRISON
      }
    })
    ...
    assert.equal(g.assemblyOpened, true, 'because the Assembly reconvened in Sivas')
  })
})
...
describe('the endgame can still be won or lost outright', () => {
  test('taking the whole map after refusing is the total-conquest ending', () => {
    ...
    drive(g, FINAL, {
      terms: 'reject',
      onRound: game => {
        if (game.round !== CONFERENCE + 1) return
        for (const t of game.territories) if (t.faction !== turkey(game)) give(game, t.slug, turkey(game)).troops = GARRISON
      }
    })
    assert.equal(g.phase, 'gameover')
  ...
  test('the landings can finish Turkey off', () => {
    ...
    g.round = CONFERENCE
    g.refuseTerms()
    g.landOn(faction(g, 'Britain'), g.bySlug['izmir'], LANDING_FIRST_WAVE)
  ...
  test('beachheads outside the Pact do not stand between Turkey and the peace', () => {
    ...
    g.round = CONFERENCE
    g.refuseTerms()
    g.landOn(faction(g, 'Britain'), g.bySlug['lesbos'], 30)
    assert.ok(g.landedOn.includes('lesbos'), 'the island is theirs')
    give(g, PACT[PACT.length - 1], turkey(g)).troops = GARRISON
    advance(g, 3)
    assert.equal(g.phase, 'gameover')
    assert.equal(g.outcome?.titleKey, 'overlay.victory.title', 'the war aim is the Pact, not the map')
  ...
  test('a beachhead inside the Pact does', () => {
    ...
    g.refuseTerms()
    advance(g, 1)
    assert.notEqual(g.phase, 'gameover', 'twenty-nine is not thirty')
    assert.equal(g.pactHeldTurns, 0)
  })
})
...
describe('how the fleets come ashore', () => {
  test('at most two beaches a turn', () => {
    ...
    for (let n = 0; n < 6; n++) {
      strippedCoast(g)
    advance(g, 1)
      assert.ok(g.landedOn.length - before <= 2, `round ${g.round} landed ${g.landedOn.length - before}`)
      before = g.landedOn.length
    }
  })
  ...
  test('a wave onto a beach an ally holds is unopposed, not a battle', () => {
    ...
    g.landOn(faction(g, 'Britain'), izmir, 30)
  ...
  test('a retaken beach can be landed on again', () => {
    ...
    const sites = g.landingSites().map(s => s.slug)
  ...
  test('the Straits can fall mid-campaign and open İstanbul', () => {
    ...
    give(g, 'gelibolu', britain).troops = GARRISON
    give(g, 'canakkale', britain).troops = GARRISON
    assert.ok(g.landingSites().some(s => s.slug === 'istanbul'), 'the Marmara is open now')
  })
  ...
  test('with no Turkish coast left the waves come ashore behind their own lines', () => {
    ...
    advance(g, 3)
    assert.deepEqual(g.landedOn, [], 'nothing was taken, because nothing was Turkish')
    assert.ok(
...
describe('the restraint rule does not stay lifted', () => {
  test('completing the Pact opens the map; losing a province shuts it again', () => {
    ...
    assert.equal(g.mayAttackOutsidePact(to), true)
    give(g, 'izmir', faction(g, 'Greece')).troops = GARRISON
    assert.equal(g.mayAttackOutsidePact(to), false, 'the war aim is unmet again, so the leash is back on')
  })

  test('a Pact province is always a legal target, occupied or landed on', () => {
    const g = board(20)
    for (const slug of PACT) assert.equal(g.mayAttackOutsidePact(g.bySlug[slug]), true, slug)
  })
})
...
describe('saving in the middle of the endgame', () => {
  test('a refused war resumes refused', () => {
    ...
    const b = new Game()
    b.restore(snapshot)
    assert.equal(b.rejectedAt, CONFERENCE)
    assert.deepEqual(b.landedOn, a.landedOn)
    assert.equal(b.round, a.round)
    drive(b, FINAL + 2, { terms: 'reject' })
    assert.equal(b.phase, 'gameover', 'and still ends at the outer limit')
  })

  test('a running Pact clock resumes running', () => {
    const a = board(PACT.length)
    drive(a, 2)
    assert.ok(a.pactHeldTurns > 0, 'the clock started')
    const snapshot = JSON.parse(JSON.stringify(a.serialize()))
    const b = new Game()
    b.restore(snapshot)
    assert.equal(b.pactHeldTurns, a.pactHeldTurns)
    drive(b, 8)
    assert.equal(b.phase, 'gameover', 'the clock ran out after loading')
    assert.equal(b.outcome?.titleKey, 'overlay.victory.title')
  })
  ...
  test('beachheads survive a save', () => {
    ...
    strippedCoast(a)
    advance(a, 2)
    assert.ok(a.landedOn.length > 0)
    const b = new Game()
    b.restore(JSON.parse(JSON.stringify(a.serialize())))
    assert.deepEqual(b.landedOn, a.landedOn)
    for (const slug of b.landedOn) assert.equal(b.mayAttackOutsidePact(b.bySlug[slug]), true)
  })
})
