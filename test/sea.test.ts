...
import { applyEvent, fresh, give, faction, turkey } from './helpers'
...
describe('Mübadale transport disruption', () => {
  const disrupted = () => {
    const g = fresh()
    g.turn.configure({ round: 16, phase: 'fortify' })
    give(g, 'aydin', turkey(g))
    applyEvent(g, 'event.mubadele')
    return g
  }

  test('halves land transfers leaving or entering an affected province', () => {
    const g = disrupted()
    g.bySlug['aydin'].troops = 11
    g.bySlug['usak'].troops = 11
    assert.equal(g.movement.transferCapacity(g.bySlug['aydin'], g.bySlug['usak']), 5, 'leaving Aydın')
    assert.equal(g.movement.transferCapacity(g.bySlug['usak'], g.bySlug['aydin']), 5, 'entering Aydın')
  })

  test('leaves transfers elsewhere at normal capacity and expires next round', () => {
    const g = disrupted()
    g.bySlug['usak'].troops = 11
    g.bySlug['aydin'].troops = 11
    assert.equal(g.movement.transferCapacity(g.bySlug['usak'], g.bySlug['kutahya']), 10)
    g.turn.configure({ round: 17 })
    assert.equal(g.movement.transferCapacity(g.bySlug['aydin'], g.bySlug['usak']), 10)
  })

  test('halves sea transfers departing from or arriving at Salonica', () => {
    const outward = shipping('Greece', 9)
    outward.turn.configure({ round: 16 })
    applyEvent(outward, 'event.mubadele')
    assert.equal(outward.movement.shippable(outward.bySlug['salonica'], outward.bySlug['izmir']), 4)
    assert.equal(outward.movement.embark('salonica', 'izmir', 99), true)
    assert.equal(outward.movement.convoys[0].troops, 4)

    const homeward = shipping('Greece')
    homeward.turn.configure({ round: 16 })
    homeward.bySlug['lesbos'].troops = 9
    applyEvent(homeward, 'event.mubadele')
    assert.equal(homeward.movement.shippable(homeward.bySlug['lesbos'], homeward.bySlug['salonica']), 4)
  })
})
