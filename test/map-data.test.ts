...
describe('territory data', () => {
  ...
  test('adjacency is symmetric on the built graph', () => {
    ...
    for (const t of g.territories)
      for (const other of t.adjacent) assert.ok(other.adjacent.includes(t), `${t.slug} → ${other.slug} is one-way`)
  })
  ...
  test('the Aegean islands hang off the coast, reached by sea not by land', () => {
    const g = new Game()
    const neighbours = (slug: string) => g.bySlug[slug].adjacent.map((a) => a.slug).sort()
    // Rhodes is a dead-end off Aydın; İzmir, Antalya and Lesbos no longer touch it.
    assert.deepEqual(neighbours('rhodes'), ['aydin'])
    // Lesbos hangs off the Anatolian coast only — Western Thrace and Rhodes are cut.
    assert.deepEqual(neighbours('lesbos'), ['balikesir', 'izmir'])
    // and the cuts are symmetric: the former neighbours no longer list the islands.
    assert.ok(!neighbours('western-thrace').includes('lesbos'), 'Western Thrace lost its Lesbos link')
    assert.ok(!neighbours('antalya').includes('rhodes'), 'Antalya lost its Rhodes link')
    assert.ok(!neighbours('izmir').includes('rhodes'), 'İzmir lost its Rhodes link')
    assert.ok(neighbours('izmir').includes('lesbos'), 'but İzmir keeps Lesbos')
  })
