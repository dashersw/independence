...
describe('exchange resolution', () => {
  ...
  test('a tie always favours the defender', () => {
    ...
    for (const face of [1, 3, 6]) {
      ...
      withRandom(loaded(face), () => g.attackRound(from.slug, to.slug))
  ...
  test('dice come back sorted high to low', () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      assert.deepEqual(dice, [...dice].sort((a, b) => b - a))
    }
  })
})
...
describe('the battle result', () => {
  ...
  test('a conquest advances the dice that won it, and asks about the rest', () => {
    ...
    assert.equal(g.pendingAdvance?.to, to.slug, 'the player is asked how many follow')
    const { min, max } = g.pendingAdvance!
  ...
  test('pending is false once the attacker cannot continue', () => {
    ...
    g.beginAttack(from.slug, to.slug)
    let result = g.attackRound(from.slug, to.slug)
    while (result?.pending) result = g.attackRound(from.slug, to.slug)
