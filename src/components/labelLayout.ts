// Label placement engine: fits a name inside an arbitrary SVG shape.
//
// For each territory we rasterize its paths onto a sampling grid (canvas
// isPointInPath, clipped to the visible map frame), then:
//   1. A chamfer distance transform gives every interior cell its clearance
//      to the nearest border.
//   2. Sizes are tried top-down on a ladder. At each size, cells with
//      clearance above ~0.6em form the "thick" region whose largest
//      connected component is where the label may live.
//   3. Candidate baselines: the longest straight run at a single "latitude"
//      along 0°, ±10°, ±20°, ±30° (so a flat candidate really is level),
//      plus an unrestricted spine (farthest pair via double BFS) that may go
//      vertical. A candidate fits a size when the text's own window — not
//      the whole line — is long enough and thick enough everywhere.
//   4. Each candidate keeps the largest size that fits it; the flattest one
//      within 82% of the best achievable size wins.
// The winning line becomes a gentle quadratic arc via textPath — always with
// at least a slight bow, never more than a gentle drape.
// Shapes too small for any arc fall back to a plain centered label.
...
export interface ArcLabel {
  ...
  d: string
  size: number
  // optional per-label styling (hand-pinned); absent = CSS defaults
  fill?: string
  stroke?: string
  strokeW?: number // outline thickness in em
}
...
// hand-placed label parameterization (see LabelEditor.tsx): chord midpoint,
// angle (SVG sense), chord length, font size and perpendicular bow of the
// curve middle (− crown ∩ / + sag ∪)
export interface LabelParams {
  ...
  size: number
  bow: number
  // optional styling; omitted fields fall back to the CSS defaults
  fill?: string
  stroke?: string
  strokeW?: number // em
}

export const arcFromParams = (p: LabelParams): ArcLabel => {
  const rad = (p.ang * Math.PI) / 180
  const ux = Math.cos(rad)
  const uy = Math.sin(rad)
  // control point putting the curve's t=0.5 midpoint at bow along the normal
  const cx = p.x + -uy * 2 * p.bow
  const cy = p.y + ux * 2 * p.bow
  const f = (n: number) => Math.round(n * 10) / 10
  const arc: ArcLabel = {
    kind: 'arc',
    d: `M ${f(p.x - (ux * p.len) / 2)},${f(p.y - (uy * p.len) / 2)} Q ${f(cx)},${f(cy)} ${f(p.x + (ux * p.len) / 2)},${f(p.y + (uy * p.len) / 2)}`,
    size: p.size
  }
  if (p.fill !== undefined) arc.fill = p.fill
  if (p.stroke !== undefined) arc.stroke = p.stroke
  if (p.strokeW !== undefined) arc.strokeW = p.strokeW
  return arc
}

// real glyph metrics, measured — a flat per-character estimate overshot the
// serif's true advance by ~25-30%, capping every label well under the size
// its region actually fits (verified against the hand-placed reference set).
// Keep in sync with .territory-label .name in game.css.
const LABEL_FONT = "700 100px 'Iowan Old Style', 'Palatino Linotype', Palatino, Georgia, serif"
const MAX_SIZE = 32
const MIN_ARC_SIZE = 8.5
// visible map frame — keep in sync with VB in MapView.tsx; labels must never
// be placed in parts of a shape that the viewport crops away (e.g. Baghdad)
const FRAME = { x1: 30, y1: 0, x2: 1530, y2: 820 }

// keyed by caller-supplied cache key (language) so a language switch, which
// changes many `names` values at once, recomputes instead of serving stale
// English-fitted geometry
const cacheByKey = new Map<string, Record<string, LabelLayout>>()

// scoring knobs, collected so the dev sweep below can search them; the
// committed values are the sweep's winner, not hand-tuned per city
export const PARAMS = {
...
export function labelLayouts(names: Record<string, string>, cacheKey = 'default'): Record<string, LabelLayout> {
  ...
  for (const [slug, data] of Object.entries(mapData.territories as Record<string, any>)) {
...
function computeLayout(ctx: CanvasRenderingContext2D, data: any, name: string): LabelLayout {
  ...
  for (let s = Math.min(MAX_SIZE, upper); s >= MIN_ARC_SIZE; s *= 0.93) {
    ...
    if (fs && fits(fs, s)) {
      ...
      const score =
        s *
        0.5 *
        (1 - PARAMS.centerW * Math.min(1, opticalDist(fs, s, Math.atan2(gy - fy, gx - fx), aim) / R) ** PARAMS.centerExp)
      bestSize = Math.max(bestSize, s)
  ...
  for (let i = 1; i < spine.length; i++)
    if (cum[i] >= total / 2) {
      ...
      M = [
        spine[i - 1][0] + (spine[i][0] - spine[i - 1][0]) * t,
