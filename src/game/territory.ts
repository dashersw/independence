...
export default class Territory {
  ...
  changeControl(newFaction: Faction, round = 1) {
    // a province that changes hands is freshly taken, not dug in
    this.quietTurns = 0
    this.entrenched = 0
    this.heldSince = round
    const index = this.faction.territories.indexOf(this)
    if (index > -1) this.faction.territories.splice(index, 1)
    this.faction = newFaction
    newFaction.territories.push(this)
  }
}
