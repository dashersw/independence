...
export class CombatSystem {
  ...
  private ensureActive(context: AttackContext) {
    ...
    if (this.active && (this.active.from !== from || this.active.to !== to)) this.cancel()
  ...
  begin(fromSlug: string, toSlug: string): BattleResult | null {
    ...
    const { from, to, attacker, defender } = context
    // Choosing a target only stages the battle for the UI. The operational
    // budget and diplomatic consequences belong to the first dice exchange,
    // when the player actually commits to the attack.
    if (this.active && (this.active.from !== from || this.active.to !== to)) this.cancel()
    ...
    return {
      ...
      defenderLosses: battle?.defenderLosses ?? 0,
      troopsMoved: 0,
