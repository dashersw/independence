...
export const MOVE_SIZE = 17
export const INPUT_SIZE = STATE_SIZE + MOVE_SIZE + (LOOKAHEAD ? STATE_SIZE : 0)

export type MoveKind = 'reinforce' | 'attack' | 'fortify' | 'sail' | 'end'
...
export const stateFeatures = (game: Game, faction: Faction): number[] => {
  ...
  const atSea = game.troopsAtSea(faction)
  return [
    clamp01(game.round / 27),
    ...
    game.isPassive(faction) || game.frozen(faction) ? 1 : 0,
    ...
    clamp01(game.reinforcementsFor(faction) / 20)
  ]
}
...
export const moveFeatures = (game: Game, faction: Faction, move: Move): number[] => {
  ...
  return [
    ...
    target ? clamp01((game.round - target.heldSince) / 8) : 0,
    defenderLoses ? 1 : 0,
 ...
 */
export const afterFeatures = (game: Game, faction: Faction, move: Move): number[] => {
  const { kind, from, to } = move
  if (kind === 'end' || !from) return stateFeatures(game, faction)

  if (kind === 'reinforce') {
    from.troops++
    const after = stateFeatures(game, faction)
    from.troops--
    return after
  }
  ...
  if (kind === 'sail') {
    const moved = game.shippable(from)
    from.troops -= moved
    const after = stateFeatures(game, faction)
    from.troops += moved
    return after
  ...
  if (kind === 'attack' && to) {
    ...
    // move it across without going through changeControl: this is a hypothetical
    // and must leave no trace — no tenure stamp, no card, no game-over check
    const ownerIndex = owner.territories.indexOf(to)
    if (ownerIndex >= 0) owner.territories.splice(ownerIndex, 1)
    ...
    if (ownerIndex >= 0) owner.territories.splice(ownerIndex, 0, to)
    return after
  }

  return stateFeatures(game, faction)
}
...
export const featuresFrom = (before: number[], game: Game, faction: Faction, move: Move): number[] => {
  ...
  const after = afterFeatures(game, faction, move)
  for (let i = 0; i < STATE_SIZE; i++) out[i] = before[i]
