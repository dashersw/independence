...
if (Object.keys(territories).length !== 45)
  throw new Error(`expected 45 territories, got ${Object.keys(territories).length}`)
...
Object.entries(territories).forEach(([slug, t]) => {
  t.poly = t.paths.map((p) => {
    if ((p.match(/[mM]/g) || []).length > 1)
      throw new Error(`${slug}: multi-subpath path; poly emission expects one ring per path`)
...
writeFileSync(join(root, 'src/game/map-data.json'), JSON.stringify({ viewBox, territories, background }, null, 1))
