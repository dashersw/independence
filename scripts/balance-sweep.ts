...
const playTurkey = (game: Game) => {
  ...
  while (game.turn.phase === 'attack' && attackGuard++ < 200) {
    ...
    if (!best) break
    game.combat.blitz(best.from, best.to)
  ...
  if (game.turn.phase === 'fortify') {
    ...
    if (interior) {
      ...
      if (border) game.movement.fortify(interior.slug, border.slug, game.movement.movable(interior))
    }
...
for (let n = 0; n < GAMES; n++) {
  ...
  while (game.turn.phase !== 'gameover' && game.turn.round <= 28 && guard++ < 4000) {
    if (game.campaign.pendingDecision) {
      ...
      game.campaign.resolveDecision(
        q.id === 'event.conference'
          ? 'reject'
    ...
    const afloat = game.movement.convoys.length
    if (game.turn.currentPlayer.isHuman) playTurkey(game)
    else {
      ai.beginTurn()
      while (ai.attackStep()) {
    ...
    if (game.movement.convoys.length > afloat) crossings += game.movement.convoys.length - afloat
  }
...
for (const key of Object.keys(LABEL))
  if (endings[key]) console.log(`  ${LABEL[key].padEnd(18)} ${pct(endings[key]).padStart(6)}`)
